...
def main() -> None:
    ...
    add(r"pool the driver holds. Whole-device measurement on a separate block-size")
    add(r"sweep over the same implementations puts that understatement between")
    add(r"$1.2$ and $3.3$ times, so these columns should be read within a framework")
    add(r"rather than across rows.")
